...
- **감성 분석**: Exaone 3.5 2.4B (Ollama, 로컬 실행) — 비용 $0
...
| 지표                       | 설명                                      | Phase         |
| -------------------------- | ----------------------------------------- | ------------- |
| **개미지표** (통합 0~100)  | 돔황챠(공포)+가즈아(탐욕) 통합 지수. CNN Fear & Greed 스타일 | Phase 1 (MVP) |
| **돔황챠지수** (공포/분노) | LLM 감성 분류 기반 하락론자 비율 (내부 계산용) | Phase 1 (MVP) |
| **가즈아지수** (환희/탐욕) | LLM 감성 분류 기반 상승론자 비율 (내부 계산용) | Phase 1 (MVP) |
| **종목별 공탐지수**        | 거래량, 변동성, 풋/콜 등 시장 데이터 기반 | Phase 3       |

> **통합 개미지표 계산식**: `antIndex = gazua / (sb + gazua) * 100`
> - 0 = 극돔황챠 (극도의 공포), 50 = 중립, 100 = 극가즈아 (극도의 탐욕)
> - sb + gazua 모두 0이면 기본값 50 (중립)
> - 5단계: 극돔황챠(0-20) / 돔황챠(20-40) / 중립(40-60) / 가즈아(60-80) / 극가즈아(80-100)
> - UI: 파랑(0, 공포) → 오렌지(100, 탐욕) 그라데이션 게이지
...
- **결론: Exaone 3.5 2.4B를 Ollama로 로컬 실행 (비용 $0)**
...
- 비꼬기/풍자 판단법: 조롱 대상에 따라 반대편으로 분류
- few-shot 예시: 11개 (다양한 톤 포함)
- 컨텍스트: 현재 주가 제공 (가격 대비 상승/하락 판단용)
- 응답 형식: JSON 강제 (`format: "json"` + 프롬프트 지시)
...
#### 돔황챠지수 / 가즈아지수 / 통합 개미지표 산출

LLM 분류 결과(상승론자/하락론자)를 기반으로 지표 계산:

- **돔황챠지수** (구 ㅅㅂ지수): 하락론자 비율 기반 (높을수록 분노/공포)
- **가즈아지수**: 상승론자 비율 기반 (높을수록 환희/탐욕)
- **통합 개미지표**: `gazua / (sb + gazua) * 100` — 두 지수를 CNN Fear & Greed 스타일로 하나의 0~100 지수로 합산. UI에서는 이 통합 지수만 노출
...
#### 통합 개미지표 (0~100) — UI 노출용

```
antIndex = gazua / (sb + gazua) * 100
(sb + gazua == 0이면 기본값 50)
```
...
| `ollama` (시스템) | Exaone 3.5 2.4B 로컬 실행 (pip 아닌 시스템 설치) |
...
GET /api/stocks                              # 추적 종목 목록 (antIndex 포함)
GET /api/stocks/:code/ant-index              # 종목 통합 개미지표 현재값
GET /api/stocks/:code/ant-index/history      # 종목 통합 개미지표 히스토리 (?period=7d|30d|90d)
GET /api/stocks/:code/sb                     # 종목 돔황챠지수 현재값 (레거시)
GET /api/stocks/:code/sb/history             # 종목 돔황챠지수 히스토리 (레거시)
GET /api/stocks/:code/gazua                  # 종목 가즈아지수 현재값 (레거시)
GET /api/stocks/:code/gazua/history          # 종목 가즈아지수 히스토리 (레거시)
...
- 브랜드 포인트: 블랙 + 오렌지 (머니그라피 톤)
- 돔황챠(공포): 파랑 계열 (#217cf9 light / #5ba3fc dark)
- 가즈아(탐욕): 오렌지 계열 (#ff6600 light / #ff9364 dark)
- 통합 개미지표 게이지: 파랑→오렌지 SVG 그라데이션
- 다크 모드 배경: 블랙/딥 차콜
...
1. **감성 분석**: 삼성전자 종목토론실 10개 글로 Exaone 2.4B 테스트 → 9/10 정확도 확인 완료 (2026.03)
2. **크롤링**: 네이버증권 종목토론실 글 목록 + 본문 크롤링 확인 완료 (2026.03)
3. **지표 계산**: 시드 데이터로 raw_score → normalized 계산 결과 검증
4. **E2E**: 크롤링 → 감성 분류 → 지표 계산 → API 응답 → 대시보드 렌더링 전체 파이프라인

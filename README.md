...
각 글마다 좋아요 수 기반 가중치를 부여한다. 공감받은 의견에 더 높은 비중을 주되, log로 감쇠시켜 극단적 쏠림을 방지한다.

```
가중치 = 1 + log10(like_count + 1)
```
...
최근 24시간 글을 대상으로, LLM 감성분석 결과(BULL/BEAR/NEUTRAL)별 가중치를 합산한다.

| 감성 | 합산 |
|------|------|
| BULL (낙관) | `bullWeight` = BULL 글들의 가중치 합 |
| BEAR (비관) | `bearWeight` = BEAR 글들의 가중치 합 |
| 전체 | `totalWeight` = 모든 글(BULL+BEAR+NEUTRAL)의 가중치 합 |

### 지표 산출

| 지표 | 공식 | 범위 | 의미 |
|------|------|------|------|
| **ㅅㅂ지수** | `bearWeight / totalWeight × 100` | 0~100 | 높을수록 공포/분노 |
| **가즈아지수** | `bullWeight / totalWeight × 100` | 0~100 | 높을수록 환희/탐욕 |
| **통합 개미지표** | `bullWeight / (bullWeight + bearWeight) × 100` | 0~100 | NEUTRAL 제외, 순수 심리 비율 |

ㅅㅂ/가즈아 지수는 NEUTRAL 포함 전체 대비 비율이고, **통합 개미지표**는 NEUTRAL을 빼고 BULL vs BEAR만의 비율이라 시장 심리를 더 명확하게 보여준다.
